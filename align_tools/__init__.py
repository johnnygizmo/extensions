...
class OBJECT_OT_align_face_to_face(bpy.types.Operator):
    ...
    bl_label = "Align Active Face to Other Face"
    bl_description = (
    ...
    match_edge_rotation: bpy.props.BoolProperty(
        ...
        description=(
            "If enabled, use the selected-edge indices instead of twist angle to align edge directions."
        ),
        default=False,
    )

    edge_index_active: bpy.props.IntProperty(
        name="Active Edge Number",
        description="Actual mesh edge index (use the mesh's edge number) on the active object",
        default=0,
        min=0,
    )

    edge_index_other: bpy.props.IntProperty(
        name="Other Edge Number",
        description="Actual mesh edge index (use the mesh's edge number) on the other object",
        default=0,
        min=0,
    )
    ...
    def execute(self, context):
        ...
        # Helper: read selected edges on an object and return dict {edge.index: world-space direction}
        def read_selected_edges_world(obj):
            ctx = bpy.context
            prev_active = ctx.view_layer.objects.active
            if ctx.mode != 'OBJECT':
                bpy.ops.object.mode_set(mode='OBJECT')
            ...
            for e in edges:
                ...
                vec = (v2 - v1)
                vec_world = mw3 @ vec
                # use the mesh edge's index as the key
                vecs[e.index] = vec_world
            bpy.ops.object.mode_set(mode='OBJECT')
            ctx.view_layer.objects.active = prev_active
            return vecs

        # Determine twist angle either from selected edges or from the twist_angle property
        twist_rad = 0.0
        if self.match_edge_rotation:

            edges_active = read_selected_edges_world(active)
            edges_other = read_selected_edges_world(other)
            if not edges_active:
                self.report({'ERROR'}, f"No selected edges on active object '{active.name}'.")
                return {'CANCELLED'}
            if not edges_other:
                self.report({'ERROR'}, f"No selected edges on other object '{other.name}'.")
                return {'CANCELLED'}

            # Use actual mesh edge numbers provided by the user
            if self.edge_index_active not in edges_active:
                self.report({'ERROR'}, f"Edge {self.edge_index_active} is not selected on active object '{active.name}'.")
                return {'CANCELLED'}
            if self.edge_index_other not in edges_other:
                self.report({'ERROR'}, f"Edge {self.edge_index_other} is not selected on other object '{other.name}'.")
                return {'CANCELLED'}

            edge_active_world = edges_active[self.edge_index_active]
            edge_other_world = edges_other[self.edge_index_other]
